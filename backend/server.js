...
// ✅ MongoDB 연결
// ===============================
mongoose.connect("mongodb://192.168.75.103:27017/errorArch")
...
app.get("/auth/kakao", (req, res) => {
  const redirectUri = "http://192.168.75.102:3000/auth/kakao/callback";
...
app.get("/auth/kakao/callback", async (req, res) => {
  ...
  try {
    ...
    const tokenResponse = await axios.post(tokenUrl, null, {
      params: {
        ...
        redirect_uri: "http://192.168.75.102:3000/auth/kakao/callback",
        code
      },
  ...
  {
    userId: user.userId,
    name: encodeURIComponent(user.name),   // 🔹 여기에 인코딩 추가!
    email: user.email
  },
  ...
  { expiresIn: "1h" }
    ...
    // 5️⃣ 프론트로 리디렉션
    res.redirect(`http://192.168.75.101/loginSuccess.html?token=${token}`);
...
app.get("/auth/naver", (req, res) => {
  const redirectUri = "http://192.168.75.102:3000/auth/naver/callback";
...
app.listen(3000, "0.0.0.0", () => {
  console.log("✅ Backend running on http://192.168.75.102:3000");
});
